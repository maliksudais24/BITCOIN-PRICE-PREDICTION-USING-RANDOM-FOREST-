...
def train_model():
    # \"\"\"Fetch new data, train model, and save to bitcoin_model.joblib\"\"\"
    df = yf.download("BTC-USD", period="1d", interval="1m", progress=False)
    if df.empty:
        print("No data retrieved. Skipping training.")
        return
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)
    df = df.reset_index()
    ...
    df = df.dropna()
    if len(df) == 0:
        print("No valid data after preprocessing. Skipping training.")
        return
    X = df[["lag1", "lag2", "price", "hour", "minute", "dayofweek"]]
    y = df["target"]
    model = RandomForestRegressor(n_estimators=200, random_state=42)
    model.fit(X, y)
    joblib.dump(model, "bitcoin_model.joblib")
    print("Model retrained and saved to bitcoin_model.joblib")
